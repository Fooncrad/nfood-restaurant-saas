...
describe("NFOOD global loading screen", () => {
  ...
  it("follows NFOOD letters then one orbit before completion", () => {
    expect(loader).toContain('const LETTERS = ["N", "F", "O", "O", "D"];');
  ...
  it("supports accessible status and reduced motion", () => {
    ...
    expect(loader).toContain('label: "جارٍ تحميل NFOOD"');
    expect(loader).toContain('label: "Chargement de NFOOD"');
    expect(loader).toContain('label: "Loading NFOOD"');
